...
Value_Public_url="https://www.twse.com.tw/exchangeReport/STOCK_DAY_AVG?response=json&date="
Value_Overthecounter_url="https://www.tpex.org.tw/web/stock/aftertrading/daily_trading_info/st43_result.php?l=zh-tw&d=108/11/01&stkno=5478"
Date_Chips_url = "https://www.tdcc.com.tw/smWeb/QryStockAjax.do"
def CrawlPublic(data,url,stock):
    res = requests.get(url+data+"&stockNo="+stock).json()
    #public=BeautifulSoup(res,'html.parser')
    print(res['data'])

def crawlStoclNo(url,Date):

    res = requests.get('https://isin.twse.com.tw/isin/C_public.jsp?strMode=4').content
    print(res)
def SqliteCreateTable(StockNo):
    String= ["PERSON","STOCK","PERCENT"]
    Rowname=[]
    for f in range(1,16):
        for e in String:
            Rowname.append(e+str(f))
    ...
    c = conn.cursor()
    try:
        c.execute('CREATE TABLE NO' + StockNo+'''(
            "DATE" INTEGER,
            "PERSON1" INTEGER,
            "STOCK1" INTEGER,
            "PERCENT1" INTEGER,
        ...
        print('table NO'+StockNo+' already exists')
    conn.commit()
    conn.close()
...
def CrawlDate(url):
    ...
    web_data = session.post(url, data={'REQ_OPR': 'qrySelScaDates'})
    print(web_data.text)
    res = json.loads(web_data.text)
    data=[]
    for a in res:
        data.insert(0,a)
    return data

def CrawlChips(url,Date,StockNo):
    conn = sqlite3.connect(r'C:\Users\mick7\OneDrive\桌面/Stock.db')
    c = conn.cursor()
    ##date compare with SQlite
    compare = c.execute("SELECT DATE  from NO"+StockNo)
    comparelist=[]
    try:
        for z in compare:
            comparelist.append(str(z[0]))
    except:
        print(StockNo+'no data')
    Date=list(set(Date)-set(comparelist))
    ##start crawl
    ...
    for a in Date:
        ...
        res = session.post(url, params=FormData,verify=False).content
        ChipsData = BeautifulSoup(res, 'html.parser')
        Listform = ChipsData.find_all('td', attrs={'align': 'right'})
            #Listform = list(Listform.pop(1).stripped_strings)
            #print(Listform)
            #Chips.append(Listform[0].strings)
        Chips = []
        Chips.append(a)
        SkipNumber=0
        for d in Listform:
            if(len(Listform) == 51):
                if(SkipNumber<45 or SkipNumber>47 ):
                    Chips.append(d.text)
...
print('CRAWL START DONNOT TURN OFF THE POWER.(SETTING SLEEP TIME OF DISPLAY)')

Date= CrawlDate(Date_Chips_url)
for a in Public_stocks:
    SqliteCreateTable(a)
    Chips=CrawlChips(Date_Chips_url,Date,a)
for a in Overthecounter_stocks:
    SqliteCreateTable(a)
    Chips=CrawlChips(Date_Chips_url,Date,a)
#SqliteCreateTable('1101')
#Chips=CrawlChips(Date_Chips_url,Date,'1101')
#crawlStoclNo(Date_Chips_url,Date)
#CrawlPublic('20191101',Value_Public_url,"1101")